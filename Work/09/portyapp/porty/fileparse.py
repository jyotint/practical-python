...
def parse_csv(
    ...
    if isinstance(lines, str):
        raise RuntimeError('"lines" should not be string!')
    ...
    try:
        ...
        for rowno, row in enumerate(rows, start=start_rowno):
            ...
            try:
                ...
                if selected_indices:
                    record = [ record[index] for index in selected_indices ]

                # Apply type conversion to the row
                if types:
                    record = [ func(data) for func, data in zip(types, record)]

                # Make a dictionary (if it has headers) or make a tuple 
                if has_headers == True:
                    record = dict(zip(selected_headers, record))
                else:
                    record = tuple(record)
                records.append(record)

            except ValueError as ex:
                _append_errored_record(errored_records, ex, rowno, record)
                if log_errors:
                    log.warning(f"  >>>              Bad Data >> Row #: {rowno}, Data: '{row}'")
                    log.debug  (f"  >>> ValueError:: Bad Data >> Exception: '{ex}'")
            except Exception as ex:
                _append_errored_record(errored_records, ex, rowno, record)
                if log_errors:
                    log.warning(f"  >>>             Catch ALL Exceptions >> Type: Row #: {rowno}, Data: '{row}'") 
                    log.debug  (f"  >>> Exception:: Catch ALL Exceptions >> Type: Exception: '{type(ex).__name__}', '{ex}'") 

    except Exception as ex:
        log.warning(f"  >>> parse_csv()::            Catch ALL Exceptions >> Type: Row #: {rowno}, Data: '{row}'") 
        log.debug  (f"  >>> parse_csv()::Exception:: Catch ALL Exceptions >> Type: Exception: '{type(ex).__name__}', '{ex}'") 
        raise   # Rethrow the caught exception

    return (records, errored_records)


def _append_errored_record(
